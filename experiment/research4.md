...
coding=utf-8 
shuyu = {'append':'将元素添加到列表末尾','insert':'可以将元素插在指定位置','pop':'删除列表中指定的任意元素'}

for key,value in shuyu.items():
    print(f"{key}:{value}")


cities = {'shanghai':{'country':'China','population':'ten million','fact':'modu'},
          'beijing':{'country':'China','population':'eleven million','fact':'gugong'},
          'shenzhen':{'country':'China','population':'twelve million','fact':'666'}
          }
...
class Car:
    def __init__(self, make, model, year):
        ...
        self.model = model
        self.year = year
        self.odometer_reading = 0    # 有些属性可以没有形参来定义，直接init方法中为其指定默认值
    ...
    def update_odometer(self, mileage):             #l 通过方法修改属性的值
...
class ElectricCar(Car):
    ...
    def fill_gas_tank(self):    # 在子类中定义一个与要重写的父类方法同名的方法，这样python就会忽略这个父类的方法，只执行子类中定义的响应方法
        """ 电动汽车没有邮箱"""
